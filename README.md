...
Project Aphelion treats this as what it is: a bounded, measurable, auditable decision problem where AI helps with judgment but never holds the pen on money.
...
Project Aphelion is intentionally layered so that facts and money stay deterministic and the LLM only reasons within a set of choices it did not create.
...
Project Aphelion implements the real Razorpay contracts and runs in two modes:
...
- Recovery probability model: ROC-AUC about 0.70, PR-AUC about 0.62 against a base rate of about 0.38, calibration error (ECE) about 0.05.
- Targeting precision: the cases the model ranks in the top 10 percent by recovery probability convert at about 76 percent, about twice the base rate. The model finds the payers.
- Value: Project Aphelion recovers essentially the same revenue as contacting every failed payment, using about a quarter fewer contacts, at a higher recovered-per-contact, while contact-everyone also contacts opted-out and fraud-flagged customers that Project Aphelion excludes.
...
Project Aphelion is built to fail safely. Covered and tested:
...
DATABASE_URL=postgres://postgres:postgres@localhost:5432/aphelion DB_DRIVER=postgres npm run migrate -w @aphelion/api
...
4. Trigger a test failed payment. Project Aphelion opens a case, decides, and creates a real test Payment Link. Never uses live credentials or real money.

Live Test Mode validation status: the integration path and the signature and payload contracts are implemented and unit tested, but a full end-to-end run against live Razorpay Test Mode is pending merchant test credentials and has not been performed here. The default `mock` mode exercises the identical pipeline offline.

## Configuration

See [.env.example](.env.example). Key variables: `DB_DRIVER`, `DATABASE_URL`, `RAZORPAY_MODE`, `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `RAZORPAY_WEBHOOK_SECRET`, `LLM_PROVIDER` (`mock` or `gemini`), `GEMINI_API_KEY`, `INTERVENTION_COST_PAISE`, and the `RATE_LIMIT_*` limits. Secrets come from the environment only and are never logged.

## Documentation

- [DEMO.md](DEMO.md): a deterministic 5-minute walkthrough of the full loop.
- [APPLICATION_NOTES.md](APPLICATION_NOTES.md): what this is, what is real versus synthetic, and the engineering highlights.
- [ARCHITECTURE.md](ARCHITECTURE.md): layers, event flow, state machines, and the real failure story.
- [SECURITY.md](SECURITY.md): trust boundaries, bounded autonomy, and the safety model.
- [EVALUATION.md](EVALUATION.md): held-out results, baselines, and the ablation (generated by `npm run eval`).
