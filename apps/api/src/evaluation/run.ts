...
async function main(): Promise<void> {
  ...
  const policyRows = ['no_action', 'contact_all', 'rules', 'ml_only', 'recoverai'].map((name) =>
  ...
  console.log('[eval] recoverai net recovered:', formatInr(policyRows.find((r) => r.name === 'recoverai')!.net));
  console.log('[eval] recoverai recovery rate (value):', pct(policyRows.find((r) => r.name === 'recoverai')!.recoveryRateValue));
}
...
function buildMarkdown(r: EvalResult): string {
  const m = r.model;
  const lines: string[] = [];
  lines.push('# RecoverAI Evaluation');
  lines.push('');
  ...
  lines.push('');
  lines.push('RecoverAI recovers essentially the same revenue as contacting everyone, using fewer');
  lines.push('contacts, while contact_all and ml_only also contact opted-out and fraud-flagged');
  lines.push('customers that RecoverAI deliberately excludes. The rules baseline is safe but leaves');
  ...
  lines.push('- recoverai: rank by calibrated expected recovery (probability times amount)');
  lines.push('');
  ...
  const full = r.policies.find((p) => p.name === 'recoverai')!;
  ...
  lines.push(`| Full RecoverAI (ML + EV + policy) | ${full.contacts} | ${formatInr(full.recoveredValue)} | ${formatInr(full.net)} | ${formatInr(Math.round(full.recoveredPerContact))} |`);
  lines.push('');
